...
"""
def generate_complaint_with_fix_order(id, order, length):
    if length < order:
...
def generate_complaint_equal_order_for(ids):
    ...
    results = []
    for id in ids:
        results.append(generate_complaint_with_fix_order(id, order, random.randint(order + 1, 30)))
    return results
...
def generate_complaints(num_complaints, constraints):
    ...
    for constraint in constraints:
        ...
        if type(constraint.ids) is list:
            ...
            order = get_unfair_order()
            for _ in range(rows):
                complaints.append(generate_complaint_with_fix_order(constraint.ids, order, random.randint(order + 1, 30)))
    for _ in range(remaining_complaints):
        complaints.append(generate_complaint_with_fix_order(0, -1, random.randint(15, 30)))
    shuffle_rounds = 2
    for _ in range(shuffle_rounds):
        random.shuffle(complaints)
    return list(map(lambda c: ";".join(c), complaints))
